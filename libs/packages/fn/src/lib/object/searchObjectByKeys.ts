 ...
 * @param path
 */
export function searchObjectByKeys(
  node: any,
  _searchParam: any,
  path = ''
): any[] {
    ...
    return (<any[]>node).reduce(
      (result: ISearchObjectByKeysResult[], item: any, index: number) => {
        return result.concat(
          searchObjectByKeys(item, _searchParam, path + '[' + index + ']')
        );
      },
      []
    );
        ...
        if (Array.isArray(_searchParam)) {
          if (isObject(node[key]) || Array.isArray(node[key])) {
            ...
            if (_searchContains(_searchParam, key) && hasValue(node[key])) {
              const _result: ISearchObjectByKeysResult =
                _findPropertiesResultObject();
              _result.key = key;
          ...
          const _callResultTruthy: boolean = _searchParam.apply(null, [
            node[key],
            key
          ]);
