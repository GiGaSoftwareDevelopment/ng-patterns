...
export function allValuesMatch(src: any, tar: any, config?: IAllValuesMatchConfig): boolean {
  if (isDefined(src) && isDefined(tar)) {
    ...
    const includeArrays: undefined | boolean = config && hasValue(config.includeArrays) ? config.includeArrays : false;
    const exludeKeys: string[] | undefined = config && hasValue(config.excludeKeys) ? config.excludeKeys : [];
