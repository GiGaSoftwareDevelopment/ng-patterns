...
async function getUpdatePackageVersion(_updatePackageJsonVersion: undefined | boolean): Promise<boolean> {
  return new Promise((resolve, reject) => {
    ...
    if (_updatePackageJsonVersion === undefined) {
      ...
      rl.question(`Do you want to update versions of packages? y|n `, (_updatePackageVersion: string) => {
        rl.close();
        resolve(_updatePackageVersion === 'y');
      });
    } else {
...
async function getUpdatePeerDependencies(_updatePeerDependencies: undefined | boolean): Promise<boolean> {
  return new Promise((resolve, reject) => {
    ...
    if (_updatePeerDependencies === undefined) {
      ...
      rl.question(`Do you want to update version in peer dependencies? y|n `, (_updatePackageVersion: string) => {
        rl.close();
        resolve(_updatePackageVersion === 'y');
      });
    } else {
...
async function start() {
  ...
  const updatePackageVersion = await getUpdatePackageVersion(argv?.updatePackageVersion);
  const updatePeerDependencies = await getUpdatePeerDependencies(argv?.updatePeerDependencies);
  updateVersions(version, updatePackageVersion, updatePeerDependencies);
}


function updateVersions(version: string, updatePackageVersion: boolean, updatePeerDependencies: boolean) {
  const publishNext = createBuilder([
    [ 'Update package.json', createPackageJsonBuilder(version, updatePackageVersion, updatePeerDependencies) ]
 ...
 */
function createPackageJsonBuilder(version: string, updatePackageVersion: boolean, updatePeerDependencies: boolean) {
  ...
  return async () => {
    glob
      ...
      .map((file: string) => {
        ...
        if (
          (updatePackageVersion && pkg?.version && pkg?.name?.startsWith('@ngpat')) ||
