import {NgPatQueueItem} from '../../../libs/packages/utils/src/lib/process-queue';
...
function createUiUxQueueItemConfig(
  key: string
): NgPatQueueItem<PackageJsonConfig> {
  return <NgPatQueueItem<PackageJsonConfig>>{
    type: 'package.json',
 ...
 */
function filterPublishablePackages(dictionaryObject: {
  [key: string]: boolean;
}): string[] {
...
export const materialConfig: NgPatQueueItem<PackageJsonConfig> =
  createUiUxQueueItemConfig('material');

export const dependencyVersionPackageList: NgPatQueueItem<PackageJsonConfig>[] =
  publishablePackageList.map(createUiUxQueueItemConfig);

export const publishableLevel_5: string[] = filterPublishablePackages(
  publishableLevel_5_Dict
).map(createPackageName);
export const publishableLevel_4: string[] = filterPublishablePackages(
  publishableLevel_4_Dict
).map(createPackageName);
export const publishableLevel_3: string[] = filterPublishablePackages(
  publishableLevel_3_Dict
).map(createPackageName);
export const publishableLevel_2: string[] = filterPublishablePackages(
  publishableLevel_2_Dict
).map(createPackageName);
export const publishableLevel_1: string[] = filterPublishablePackages(
  publishableLevel_1_Dict
).map(createPackageName);
export const publishableLevel_0: string[] = filterPublishablePackages(
  publishableLevel_0_Dict
).map(createPackageName);
